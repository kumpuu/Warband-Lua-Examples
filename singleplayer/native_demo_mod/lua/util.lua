...
function getMyAgent()
    ...
    if game.game_in_multiplayer_mode() then
        myAgent = game.player_get_agent_id(game.multiplayer_get_my_player())
    else
        myAgent = game.get_player_agent_no()
    end

    if game.agent_is_active(myAgent) then return myAgent end
end

function getHorse(agent)
    local horse = game.agent_get_horse(agent)
    if game.agent_is_active(horse) then
        return horse
    end
end

--Sometimes we want the horse instead of rider
function getMyAgentOrHorse()
    local myAgent = getMyAgent()
    if myAgent then
        return getHorse(myAgent) or myAgent
    end
end
...
function find_or_create_scene_prop(kind, pos, scale)
    ...
    if not prop then
        ...
        prop = game.reg0
        game.scene_prop_set_slot(prop, scene_prop_was_spawned, 1)
    end

    game.scene_prop_set_slot(prop, scene_prop_slot_in_use, 1)

    if pos then
        game.prop_instance_set_position(prop, pos)
    end
